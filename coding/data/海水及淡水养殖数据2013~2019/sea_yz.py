...
def saveData(city, year, kind, num):
    ...
    sql = "insert into sea_yz( city, year, kind, num) values ('%s',%d,'%s',%d)" %(city,(year),kind,(num))
    # 执行sql
    print(sql)
    cur.execute(sql)
    # 保存修改内容
...
time.sleep(1)
p_input2 = driver.find_element(By.XPATH, '//*[@id="368"]').click()
time.sleep(1)
...
p_input2 = driver.find_element(
    By.XPATH, '//*[@id="ItemsList"]/dd[13]/input').click()
p_input2 = driver.find_element(
    By.XPATH, '//*[@id="ItemsList"]/dd[22]/input').click()
p_input2 = driver.find_element(
    By.XPATH, '//*[@id="ItemsList"]/dd[32]/input').click()
p_input2 = driver.find_element(
