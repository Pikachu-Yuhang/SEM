...
def saveData(city, year, kind, num):
    ...
    sql = "insert into yycz_nj( city, year, kind, num) values ('%s',%d,'%s',%d)" % (
        city, (year), kind, (num))
    # 执行sql
    print(sql)
    cur.execute(sql)
...
p_input2 = driver.find_element(
    By.XPATH, '//*[@id="ItemsList"]/dd[3]/input').click()
p_input2 = driver.find_element(
    By.XPATH, '//*[@id="ItemsList"]/dd[4]/input').click()
p_input2 = driver.find_element(
    By.XPATH, '//*[@id="ItemsList"]/dd[5]/input').click()
p_input2 = driver.find_element(
...
# print(listData)
city = '全国总计'
saveData(city, 2013, '渔业总产值（万元）',  cn1_2013)
saveData(city, 2014, '渔业总产值（万元）',  cn1_2014)
saveData(city, 2015, '渔业总产值（万元）',  cn1_2015)
saveData(city, 2016, '渔业总产值（万元）',  cn1_2016)
saveData(city, 2017, '渔业总产值（万元）',  cn1_2017)
saveData(city, 2018, '渔业总产值（万元）',  cn1_2018)
saveData(city, 2019, '渔业总产值（万元）',  cn1_2019)
